import type {Participant, Room} from "../types/room.types";
import {log} from "../logging";
import {getConfig} from "../config";
import {v4 as uuidv4} from 'uuid';
import crypto from 'crypto';
import {AddParticipantResult} from "../types/webrtc.types";
import {TurnServerCredentials} from "../types/network.types";
...
export class RoomManager {
    ...
    createRoom(roomId: string, creatorSocketId: string): Room {
        ...
        // Check room limits
        if (this.rooms.size >= this.cfg.room.maxRooms) {
            log('error', `Maximum number of rooms reached`, { maxRooms: this.cfg.room.maxRooms });
        ...
        const room: Room = {
            ...
            maxParticipants: this.cfg.room.maxParticipants,
            lastActivity: "",
            timeoutDuration: this.cfg.room.timeoutDuration,
            isActive: true
        };
        ...
        for (const [roomId, room] of this.rooms.entries()) {
            ...
            for (const [participantKey, participant] of room.participants.entries()) {
                if (!participant.isConnected) {
                    ...
                    if (disconnectAge > this.cfg.room.participantReconnectionWindow) {
    ...
    generateTurnCredentials(userName: string, ttl: number = 3600): TurnServerCredentials {
        ...
        if (this.cfg.turnServer.secret === undefined) {
        ...
        const hmac = crypto.createHmac('sha256', this.cfg.turnServer.secret);
        hmac.update(usr);
        ...
        return {
            ...
            ttl: ttl,
            urls: [
                `turn:${this.cfg.turnServer.url}:${this.cfg.turnServer.port}`,
                `stun:${this.cfg.turnServer.url}:${this.cfg.turnServer.port}`
            ]
        }
    }
    ...
    private startCleanupInterval(): void {
        this.cleanupInterval = setInterval(() => {
            this.performCleanup();
        }, this.cfg.room.cleanupInterval);

        log('info', `Room cleanup interval started`, {
            intervalMs: this.cfg.room.cleanupInterval,
            roomTimeoutMs: this.cfg.room.timeoutDuration,
            reconnectionWindowMs: this.cfg.room.participantReconnectionWindow
        });
    }
}
